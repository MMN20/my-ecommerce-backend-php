...
$sql = "select count(orders_id) as ordersCount from orders where orders_statusid = 2";
...
left join coupons on ordersview.orders_couponid = coupons.coupons_id where orders_statusid = 2 and orders_id > $currentIndex order by orders_id asc limit 3";
$stmt = $con->prepare($sql);
$stmt->execute();
